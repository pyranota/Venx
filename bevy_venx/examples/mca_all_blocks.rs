...
fn setup(
    ...
    let plat = VenxPlat::load("mca_mid_all").unwrap_or_else(|e| {
        warn!("Plat wasnt found on device, creating new and saving ({e})");
        // Convert from minecraft map
        let plat = VenxPlat::load_mca_untyped("./assets/mca/4/", (0..2, 0..2)).unwrap();
        plat.save("mca_mid_all").unwrap();
        plat
    });

    for mesh in plat.static_mesh(0..32, 3..6, 0..32, Some(0)) {
